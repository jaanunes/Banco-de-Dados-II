...
-- 4
SELECT nome_func, salario, id_depto FROM funcionario 
WHERE id_depto IN (2,3)
  AND salario BETWEEN 800 AND 1200;
  
-- 5 
SELECT f.nome_func, f.endereco FROM funcionario f
JOIN departamento d ON f.id_depto = d.id_depto
WHERE d.nome_depto = 'Pesquisa';

-- 6
SELECT nome_func, DATE_FORMAT(data_nasc, '%d/%m/%y') AS nascimento
FROM funcionario WHERE nome_func = 'Joao Silva';

-- 7
SELECT nome_func FROM funcionario WHERE id_superv IS NULL;

-- 8
SELECT f.nome_func, d.nome_depto, p.nome_proj FROM funcionario f
JOIN departamento d ON f.id_depto = d.id_depto
